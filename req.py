...
def course_id():
	...
	url2 = "https://saral.navgurukul.org/api/courses" + "/" +str(id_list[user_id])+ "/" + "exercises"
	restext = saral_url(url2)
	dic_data = json.loads(restext)
	return ([dic_data["data"],id_list[user_id]])

idlistdata = course_id()
...
def Third_api():
	url3 = "http://saral.navgurukul.org/api/courses/" + str(user_id) + "/" + "exercise/getBySlug?slug=" + str(slug_list[content])
	text_content = saral_url(url3)
	dict_type_slug = json.loads(text_content)
	print(dict_type_slug['content'],"\n")
Third_api()

for i in range(len(slug_list)):
	next = input("enter the n/p for next and previous or next content: ")
	if next == "n":
		content = content+1
